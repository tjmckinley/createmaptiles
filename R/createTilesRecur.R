...
createTilesRecur <- function(object, start_coords, zoom, title, max_zoom, tms)
{
	...
	{		
		...
		dir.create(file.path(title, zoom))
		#create sub-directories based on x- and y-coordinates
		for(i in unique(coords[, 1])) dir.create(file.path(title, zoom, i))
	
		#now plot into directories
		for(i in 1:nrow(coords))
		{
			pathtofile <- file.path(title, zoom, coords[i, 1])
			png(file.path(pathtofile, paste0(coords[i, 2], ".png")), width = 248, height = 248, bg = 'transparent')
